...
import com.company.user.config.KafkaProperties;
...
class RafKafkaResourceIT {
    ...
    void setup() {
        ...
        RafKafkaResource kafkaResource = new RafKafkaResource(kafkaProperties);
    ...
    void producesMessages() throws Exception {
        restMockMvc.perform(post("/api/raf-kafka/publish/topic-produce?message=value-produce"))
    ...
    void consumesMessages() throws Exception {
        ...
        MvcResult mvcResult = restMockMvc.perform(get("/api/raf-kafka/consume?topic=topic-consume"))
            .andExpect(status().isOk())
